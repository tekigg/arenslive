...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(streamerIds)) {
      return NextResponse.json({ error: 'Invalid input: streamerIds must be an array' }, { status: 400 });
    }
    ...
    const clientId = homeData.substring(homeData.indexOf(start) + start.length, homeData.lastIndexOf(end)).trim();

    const url = 'https://gql.twitch.tv/gql';

    const results = await Promise.all(streamerIds.map(async (streamerId: string) => {
      const payload = [{
